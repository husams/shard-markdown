...
@click.option(
    '--section',
    help='Show specific configuration section only'
)
@click.pass_context
...
@click.option(
    '--template',
    help='Use configuration template (future feature)'
)
@click.pass_context
...
def path(ctx):
    ...
    """
    console.print("[blue]Configuration file locations (in order of precedence):[/blue]\n")
    
    for i, location in enumerate(DEFAULT_CONFIG_LOCATIONS, 1):
        exists = "✓" if location.exists() else "✗"
        status = "[green]exists[/green]" if location.exists() else "[dim]not found[/dim]"
        console.print(f"  {i}. {exists} {location} ({status})")
    
    console.print("\n[dim]The first existing file will be used.[/dim]")
    console.print("[dim]Use 'shard-md config init' to create a new configuration file.[/dim]")
