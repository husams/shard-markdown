...
@click.pass_context
def search(ctx, query_text, collection, limit, similarity_threshold, include_metadata, format):
    ...
    try:
        ...
        # Perform search
        console.print(f"[blue]Searching for: '{query_text}' in collection '{collection}'[/blue]")
        ...
        # Display results
        if format == 'table':
            _display_search_results_table(results, include_metadata, similarity_threshold)
        elif format == 'json':
        ...
        console.print(f"\n[green]Found {count} document(s)[/green]")
...
@click.pass_context
def get(ctx, document_id, collection, format, include_metadata):
    ...
    try:
        ...
        # Get document
        console.print(f"[blue]Retrieving document '{document_id}' from collection '{collection}'[/blue]")
        ...
        console.print(f"\n[green]✓ Document retrieved successfully[/green]")
    ...
    for i, (doc_id, doc, distance) in enumerate(zip(ids, documents, distances)):
        ...
        if include_metadata and i < len(metadatas):
            ...
            if metadata:
                ...
                key_fields = ['source_file', 'chunk_index', 'structural_context']
                metadata_str = ', '.join([
