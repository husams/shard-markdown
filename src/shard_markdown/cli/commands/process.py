...
from ...utils.validation import validate_input_paths, validate_chunk_parameters, validate_collection_name
...
def process(ctx, input_paths, collection, chunk_size, chunk_overlap, 
           chunk_method, recursive, create_collection, clear_collection,
           dry_run, max_workers, collection_metadata, use_mock):
        ...
        if clear_collection and collection_exists:
            if click.confirm(f"Clear all documents from collection '{collection}'?"):
                try:
                    ...
                    console.print(f"[yellow]Collection clearing not implemented in mock client[/yellow]")
        
        # Get or create collection
        collection_obj = chroma_client.get_or_create_collection(
            collection, 
            create_if_missing=create_collection
        )
            ...
            if len(validated_paths) == 1:
                ...
                if result.success:
                    ...
                    chunks = processor._enhance_chunks(
                        ...
                        processor.metadata_extractor.extract_file_metadata(validated_paths[0]),
                ...
                for result in batch_result.results:
                    if result.success:
                        try:
                            ...
                            file_metadata = processor.metadata_extractor.extract_file_metadata(result.file_path)
                            doc_metadata = processor.metadata_extractor.extract_document_metadata(ast)
...
def _display_single_result(processing_result, insert_result):
    ...
    console.print(table)
    console.print(f"[green]✓ Successfully processed and stored {processing_result.chunks_created} chunks[/green]")
...
def _display_batch_results(batch_result, insert_result, verbose: int):
    ...
    if insert_result.success:
        console.print(f"[green]✓ Successfully processed {batch_result.successful_files} files and stored {insert_result.chunks_inserted} chunks[/green]")
    else:
        console.print(f"[red]✗ Processing completed but insertion failed: {insert_result.error}[/red]")