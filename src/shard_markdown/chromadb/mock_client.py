...
class MockCollection:
    ...
    def get(self, ids: Optional[List[str]] = None, include: Optional[List[str]] = None):
        """Get documents from collection."""
        if ids:
            return {
                'ids': ids,
                'documents': [self.documents[id_]['document'] for id_ in ids if id_ in self.documents],
                'metadatas': [self.documents[id_]['metadata'] for id_ in ids if id_ in self.documents]
            }
        else:
            return {
...
class MockChromaDBClient:
    ...
    def get_or_create_collection(self, name: str, create_if_missing: bool = False, metadata: Optional[Dict] = None) -> MockCollection:
    ...
    def bulk_insert(self, collection: MockCollection, chunks: List[DocumentChunk]) -> InsertResult:
        ...
        try:
            ...
            logger.info(f"Mock bulk insert: {len(chunks)} chunks in {processing_time:.2f}s")
        ...
        if self.storage_path.exists():
            try:
                ...
                for name, collection_data in data.items():
                    collection = MockCollection(name, collection_data.get('metadata', {}))
                    collection.documents = collection_data.get('documents', {})
                    collection._count = len(collection.documents)
                    self.collections[name] = collection
                
                logger.debug(f"Loaded {len(self.collections)} mock collections from storage")
