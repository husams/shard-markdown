...
class ChromaDBClient:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            raise ChromaDBError(
                f"Unexpected error connecting to ChromaDB: {self.config.host}:{self.config.port}",
                error_code=1402,
                context={
                    'host': self.config.host,
                    'port': self.config.port,
    ...
    def get_or_create_collection(self, name: str, 
        ...
        try:
            ...
            if not create_if_missing:
                raise ChromaDBError(
                    f"Collection '{name}' does not exist and create_if_missing=False",
                    error_code=1413,
    ...
    def bulk_insert(self, collection: chromadb.Collection, 
        ...
        try:
            ...
            logger.info(f"Inserted {len(chunks)} chunks into '{collection.name}' in {processing_time:.2f}s")
            ...
            logger.error(f"Bulk insert failed after {processing_time:.2f}s: {error_msg}")
    ...
    def _test_connectivity(self) -> None:
        ...
        try:
            ...
            if result != 0:
                raise NetworkError(
                    f"Cannot connect to ChromaDB server: {self.config.host}:{self.config.port}",
                    error_code=1601,
                    context={
                        'host': self.config.host,
                        'port': self.config.port,
            ...
            raise NetworkError(
                f"Connection timeout to ChromaDB: {self.config.host}:{self.config.port}",
                error_code=1603,
                context={
                    'host': self.config.host,
                    'port': self.config.port,
    ...
    def _validate_insertion_data(self, ids: List[str], documents: List[str], 
