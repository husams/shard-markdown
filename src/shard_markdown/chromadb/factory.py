...
def create_chromadb_client(config: ChromaDBConfig, use_mock: Optional[bool] = None):
    ...
    Args:
        ...
        use_mock: Whether to use mock client. If None, auto-detect based on environment
        
    Returns:
        ChromaDB client (real or mock)
    """
    # Auto-detect if we should use mock
    if use_mock is None:
        # Use mock if SHARD_MD_USE_MOCK_CHROMADB is set or if ChromaDB is not available
        use_mock = (
            os.getenv("SHARD_MD_USE_MOCK_CHROMADB", "").lower() in ("true", "1", "yes") or
            not _is_chromadb_available() or
            not _test_chromadb_connectivity(config)
        )
...
def _test_chromadb_connectivity(config: ChromaDBConfig) -> bool:
    ...
    try:
        ...
        if result == 0:
            logger.debug(f"ChromaDB server is accessible at {config.host}:{config.port}")
            return True
        else:
            logger.debug(f"ChromaDB server not accessible at {config.host}:{config.port}")
            return False
