...
class TestCLIConfigIntegration:
    ...
    @staticmethod
    def get_flat_config() -> dict:
        """Get a flat configuration structure for testing."""
        return {
            ...
            "process_pattern": "*.md",
            "process_include_frontmatter": True,
            "process_include_path_metadata": True,
            "log_level": "INFO",
    ...
    def test_cli_set_ip_address(self, temp_project_dir: Path) -> None:
        ...
        # Initial config in .shard-md/config.yaml
        initial_config_file = temp_project_dir / ".shard-md" / "config.yaml"
        # Local config file where --local saves to
        local_config_file = temp_project_dir / "shard-md.yaml"

        # Create initial config with flat structure
        initial_config = {
            "chroma_host": "localhost",
            "chroma_port": 8000,
            "chroma_ssl": False,
        ...
        with open(initial_config_file, "w") as f:
        ...
        # Read config from local file and verify
        with open(local_config_file) as f:
            config = yaml.safe_load(f)

        assert config["chroma_host"] == "192.168.1.100"
    ...
    def test_cli_set_multiple_values(self, temp_project_dir: Path) -> None:
        ...
        initial_config = self.get_flat_config()
        with open(initial_config_file, "w") as f:
            yaml.dump(initial_config, f)
        ...
        for key, value in test_values:
            ...
            # Verify the value was saved to the local config file
            with open(local_config_file) as f:
                config = yaml.safe_load(f)

            # Extract the actual value for comparison (flat config)
    ...
    def test_cli_validation_errors(self, temp_project_dir: Path) -> None:
        ...
        original_port: int = initial_config["chroma_port"]
        ...
        with open(initial_config_file) as f:
            config = yaml.safe_load(f)

        # Port should remain unchanged since validation failed
        assert config["chroma_port"] == original_port
    ...
    def test_cli_various_ip_formats(
        ...
        assert config["chroma_host"] == ip_address
    ...
    def test_cli_init_and_set_workflow(self, temp_project_dir: Path) -> None:
        ...
        # config init creates shard-md.yaml in current directory (local config)
        local_config_file = temp_project_dir / "shard-md.yaml"
        assert local_config_file.exists(), "Config file was not created"
        ...
        assert config["chroma_host"] == "203.0.113.1"
    ...
    def test_cli_set_complex_ip_addresses(self, temp_project_dir: Path) -> None:
        ...
        for ip in test_ips:
            ...
            assert config["chroma_host"] == ip, f"IP {ip} was not set correctly"
    ...
    def test_cli_env_var_override(self, temp_project_dir: Path) -> None:
        ...
        env = os.environ.copy()
        env["SHARD_MD_CHROMA_HOST"] = "10.5.5.5"
        ...
        assert "10.5.5.5" in result.stdout or "chroma_host: 10.5.5.5" in result.stdout
