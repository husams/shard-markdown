...
class TestDocumentProcessor:
    ...
    def test_process_document_success(self, processor, sample_markdown_file, sample_chunks,
                                     mock_parser, mock_chunker, mock_metadata_extractor):
        """Test successful document processing."""
        # Setup mocks
        mock_parser.parse.return_value = Mock()
        mock_chunker.chunk_document.return_value = sample_chunks
        mock_metadata_extractor.extract_file_metadata.return_value = {"file_type": "markdown"}
        mock_metadata_extractor.extract_document_metadata.return_value = {"title": "Test"}
    ...
    def test_process_document_no_chunks_generated(self, processor, sample_markdown_file,
                                                 mock_parser, mock_chunker, mock_metadata_extractor):
    ...
    def test_process_document_parsing_error(self, processor, sample_markdown_file,
                                           mock_parser):
    ...
    def test_enhance_chunks(self, processor, sample_chunks, sample_markdown_file,
    ...
    def test_batch_processing_success(self, processor, test_documents, sample_chunks,
                                     mock_parser, mock_chunker, mock_metadata_extractor):
    ...
    def test_batch_processing_partial_failure(self, processor, test_documents,
                                             mock_parser, mock_chunker, mock_metadata_extractor):
        ...
        assert result.successful_files == 0  # Both fail - one due to exception, one due to no chunks
    ...
    def test_concurrent_processing(self, processor, test_documents, sample_chunks,
                                  mock_parser, mock_chunker, mock_metadata_extractor):
        ...
        assert result.successful_files == len(file_paths)
        assert result.total_processing_time < end_time - start_time + 1  # Allow some tolerance
    
    @pytest.mark.parametrize("max_workers", [1, 2, 4, 8])
    def test_batch_processing_different_worker_counts(self, processor, test_documents,
                                                     sample_chunks, mock_parser, mock_chunker,
                                                     mock_metadata_extractor, max_workers):
        ...
        file_paths = list(test_documents.values())
        result = processor.process_batch(file_paths, "test-collection", max_workers=max_workers)
        
        assert result.successful_files == len(file_paths)
        assert result.total_chunks == len(file_paths) * len(sample_chunks)
    
    def test_processing_time_measurement(self, processor, sample_markdown_file,
                                        mock_parser, mock_chunker, mock_metadata_extractor):
