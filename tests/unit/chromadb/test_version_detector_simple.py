...
class TestBasicVersionDetector:
    ...
    def test_detector_custom_initialization(self):
        ...
        assert detector.port == 8000
        assert detector.base_url == "http://custom-host:8000"
    ...
    def test_detect_chromadb_version_function(self, mock_detector_class):
        ...
        result = detect_chromadb_version(host="test-host", port=8000)
        ...
        mock_detector_class.assert_called_once_with(
            host="test-host", port=8000, timeout=30.0, max_retries=5
        )
