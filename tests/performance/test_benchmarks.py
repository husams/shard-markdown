...
class TestProcessingBenchmarks:
    ...
    @pytest.fixture
    def benchmark_config(self) -> ChunkingParams:
        ...
        return ChunkingParams(chunk_size=10000, overlap=500, method="structure")

    def test_single_document_processing_benchmark(
        self, temp_dir: Any, benchmark_config: ChunkingParams
    ) -> None:
    ...
    def test_chunking_method_performance_comparison(self, temp_dir: Any) -> None:
        ...
        for method in methods:
            config = ChunkingParams(chunk_size=1000, overlap=200, method=method)
            processor = DocumentProcessor(config)
...
class TestMemoryEfficiency:
    ...
    def benchmark_config(self) -> ChunkingParams:
        ...
        # Use larger chunk size for performance tests to avoid validation errors
        # with generated content that has long sections
        return ChunkingParams(chunk_size=20000, overlap=500, method="structure")
